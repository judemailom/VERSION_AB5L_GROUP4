			...
			<form id="view_classlist" method="post" action="">
				...
				<?php
					...
					if(isset($_POST['delete'])){
						...
						$query1 = "select memberid from classlist_member where id=(SELECT id from classlist where clname='{$_SESSION['classlist']}')";
						...
						for($j=0;$j<sizeof($array);$j=$j+1){
							...
							if(isset($_POST['delete'][$temp])){
								//echo "HAHAHAHA";
								$query2 = "DELETE FROM classlist_member WHERE id=(SELECT id FROM classlist WHERE clname='{$_SESSION['classlist']}') AND memberid='{$array[$j]['memberid']}'";
