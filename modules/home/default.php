...
<?php
	...
	if(isset($_POST['post_announcement'])){
		...
		$result1 = performQuery($new_announcement);
		//var_dump($result1);
...
<?php
	...
	if($user_type == 'Administrator'){
		$announcements = performQuery('SELECT * FROM announcement WHERE author_id IN (SELECT user_id FROM user WHERE user_type = "Administrator");');  
		$announceNum = mysql_query('select COUNT(announcement_id) from announcement where author_id in(select user_id from user WHERE user_type="Administrator");');
